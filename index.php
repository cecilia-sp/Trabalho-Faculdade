...
require 'inc/configuration.php';/**chamar a classe**/
...
$app->get(
    ...
    function () {
        ...
        require_once("view/todos-produtos.php");
...
$app->get(
    ...
    function () {
        ...
        require_once("view/Trabalho/index.php");
...
$app->get('/produtos', function(){
    ...
    $sql = new Sql();
    $data = $sql->select("SELECT * FROM tb_produtos where preco_promocional > 0 order by preco_promocional desc limit 6;");/*trás produtos que estão em promoção*/
    foreach ($data as &$produto) {
        $preco = $produto['preco']; /*valor total do produto*/
        $centavos = explode(".", $preco); /*para pegar o valor apos o . ou seja os centavos.*/
        $produto['preco'] = number_format($preco, 0, ",", ".");
        $produto['centavos'] = end($centavos);/*pega os dados da varivel $centavos*/
        $produto['parcelas'] = 10;
        $produto['parcela'] = number_format($preco/$produto['parcelas'], 2, ",", "."); /*preco total divido por total de parcelas*/
        $produto['total'] = number_format($preco, 2, ",", ".");/*valor total do produto separado por casa de milhar . e casa decimal por ,*/
    }
...
$app->get('/produtos-mais-buscados', function(){
    $sql = new Sql();
    $data = $sql->select("
        SELECT * FROM tb_produtos where id_cat = 1 
    ");
    foreach ($data as &$produto) {
        $preco = $produto['preco'];
...
$app->get('/produtos-mais-buscados2', function(){
    $sql = new Sql();
    $data = $sql->select("
        SELECT * FROM tb_produtos where id_cat = 2
    ");
    foreach ($data as &$produto) {
        $preco = $produto['preco'];
        $centavos = explode(".", $preco);
...
$app->get('/produtos-mais-buscados3', function(){
    $sql = new Sql();
    $data = $sql->select("
        SELECT * FROM tb_produtos where id_cat = 3
    ");
    foreach ($data as &$produto) {
        $preco = $produto['preco'];
        $centavos = explode(".", $preco);
...
$app->run();  /**Inicia todo processo do app ou seja das rotas**/
